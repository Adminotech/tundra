...
set(OGRE_OVERLAY_LIBRARY $ENV{OGRE_OVERLAY_LIBRARY})
...
macro(configure_ogre)
    ...
    if (APPLE)
        if ("${OGRE_DIR}" STREQUAL "" OR NOT IS_DIRECTORY ${OGRE_DIR}/lib)
            ...
            find_library(OGRE_OVERLAY_LIBRARY OgreOverlay)
            set(OGRE_OVERLAY_DIR ${OGRE_OVERLAY_LIBRARY})
        else()
            ...
            set(OGRE_OVERLAY_LIBRARY ${OGRE_DIR}/lib/relwithdebinfo/OgreOverlay.framework)
        endif()
    elseif (UNIX)
        find_library(OGRE_LIBRARY OgreMain HINTS ${OGRE_DIR}/lib)
        find_library(OGRE_OVERLAY_LIBRARY OgreOverlay HINTS ${OGRE_DIR}/lib)
    endif()
    ...
    if (APPLE)# AND IS_DIRECTORY ${OGRE_DIR}/Headers) # OGRE_DIR points to a manually installed Ogre.framework?
        if (IS_DIRECTORY ${OGRE_DIR}/lib)
            ...
            include_directories(${OGRE_DIR}/lib/relwithdebinfo/OgreOverlay.framework/Headers)
            link_directories(${OGRE_DIR}/lib/relwithdebinfo/OgreOverlay.framework)
        else()
            include_directories(${OGRE_DIR}/Headers)
            link_directories(${OGRE_DIR})
            include_directories(${OGRE_OVERLAY_DIR}/Headers)
            link_directories(${OGRE_OVERLAY_DIR})
        endif()
    ...
    elseif (IS_DIRECTORY ${OGRE_DIR}/OgreMain) # Ogre path points to #1 above.
        ...
        include_directories(${OGRE_DIR}/Components/Overlay/include)
        if (MSVC)
            include_directories(${OGRE_DIR}/RenderSystems/Direct3D9/include)
        elseif (UNIX)
            include_directories(${OGRE_DIR}/RenderSystems/GL/include)
        endif()
    ...
    elseif (IS_DIRECTORY ${OGRE_DIR}/include/OGRE) # If include/OGRE exists, then OGRE_DIR points to the SDK (#2 above)
        ...
        include_directories(${OGRE_DIR}/include/OGRE/Overlay)
        link_directories(${OGRE_DIR}/lib)
        if (MSVC)
    ...
    # If no Boost TBB is used for Ogre threading
    ...
    if (TUNDRA_NO_BOOST)
        ...
        if (MSVC)
            ...
            link_directories(${TBB_HOME}/lib/${TBB_ARCH}/${VC_VER})
        elseif (UNIX)
            link_directories(${TBB_HOME}/lib/tbb_release)
        endif()
    endif()
...
macro(link_ogre)
    if (MSVC)
        target_link_libraries(${TARGET_NAME} debug OgreMain_d optimized OgreMain debug OgreOverlay_d optimized OgreOverlay)
        if (ENABLE_DIRECTX)
            target_link_libraries(${TARGET_NAME} debug RenderSystem_Direct3D9_d optimized RenderSystem_Direct3D9)
        endif()
    else()
        target_link_libraries(${TARGET_NAME} ${OGRE_LIBRARY} ${OGRE_OVERLAY_LIBRARY})
        if (UNIX AND TUNDRA_NO_BOOST)
            target_link_libraries(${TARGET_NAME} tbb pthread)
        endif()
    endif()
endmacro()
